...
public class OpenAiClient implements AiClient {
    ...
    private static final String OCR_PROMPT = """
            ...
            둘 중 어느 쪽인지는 사진에 실제로 인쇄된 모양만으로 판단하세요 — 성분 조합이 시중에
            흔한 복합제와 비슷해 보인다는 이유로 하나로 합치면 안 됩니다:
            - 성분명·함량이 하나의 제품명 아래 성분표(표 형태로 여러 줄)에 같이 적혀 있으면
              진짜 복합제 → 한 항목.
            - 반대로 각 성분이 서로 다른 봉지·라벨에 따로 인쇄되어 있고(각자 자기 이름과 자기
              용법 문구를 따로 가짐), 그 봉지들을 한 봉투에 같이 담아 찍은 사진이면 → 서로 다른
              약이므로 항목을 나누세요. 이때는 productName도 각 봉지에 적힌 이름을 그대로 각각
              씁니다 — 두 봉지의 성분명을 억지로 한 productName 아래 묶지 마세요.
            판단이 애매하면(봉지 경계가 사진에서 잘 안 보이는 등) note에 그 불확실성을 적어
            사용자가 확인 화면에서 직접 고칠 수 있게 하세요.
