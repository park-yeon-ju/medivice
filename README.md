...
**모든 코드 파일에 주석을 답니다.** 전원이 AI로 코드를 만들기 때문에, 주석이 없으면 아무도 그 코드를 설명할 수 없는 상태가 됩니다. 평가에 `설명 가능성` 항목이 있습니다.

- **파일 최상단** — 이 파일이 무엇을 하는지, 관련 UC·화면 ID
- **코드 안** — 문법이 아니라 **왜 그렇게 짰는지**를 한국어로
- 코드를 고치면 주석도 같이 고칩니다
- **주석 없는 코드는 머지하지 않습니다**

에이전트에게 시킬 때 이 문장을 꼭 넣으세요.

> 모든 파일 최상단에 이 파일이 뭘 하는지 요약 주석을 달고,
> 코드 안에는 왜 그렇게 짰는지 한국어 주석을 달아줘.

형식과 예시는 [`CONTRIBUTING.md` 4.7절](CONTRIBUTING.md)에 있습니다.
